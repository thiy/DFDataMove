...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="create-table">
		<xsl:element name="table">
			<xsl:variable name="TransTime" select="current()/TransactionTime" />
			...
			<xsl:for-each select="*">
				<xsl:if test="not(@customValue)">
					<xsl:element name="column">
						<xsl:attribute name="columnName"><xsl:value-of
							select="name()" /></xsl:attribute>
						<xsl:if test="@isPrimaryKey = 'true'">
							<xsl:attribute name="isPrimaryKey"><xsl:text>true</xsl:text></xsl:attribute>
						</xsl:if>
				...
				<xsl:if test="@customValue">
					<xsl:element name="column">
						<xsl:if test="@isPrimaryKey = 'true'">
							<xsl:attribute name="isPrimaryKey"><xsl:text>true</xsl:text></xsl:attribute>
						</xsl:if>
						<xsl:choose>
							<xsl:when test="@customValue ='UUID'">
								<xsl:attribute name="columnName"><xsl:value-of
									select="name()" /></xsl:attribute>
								<xsl:attribute name="withoutQuotes">true</xsl:attribute>
								<xsl:attribute name="customValue">UUID</xsl:attribute>
								<xsl:text>SYS_GUID()</xsl:text>
							</xsl:when>
							<xsl:when test="@customValue ='ACTIVE_FLAG'">
								<xsl:attribute name="columnName"><xsl:value-of
									select="name()" /></xsl:attribute>
								<xsl:text>Y</xsl:text>
							</xsl:when>
							<xsl:when test="@customValue ='END_DATE'">
								<xsl:attribute name="columnName"><xsl:value-of
									select="name()" /></xsl:attribute>
								<xsl:attribute name="withoutQuotes">true</xsl:attribute>
							...
							<xsl:when test="@customValue ='TRANSACTION_TIME'">
								<xsl:attribute name="columnName"><xsl:value-of
									select="name()" /></xsl:attribute>
								<xsl:attribute name="withoutQuotes">true</xsl:attribute>
								<xsl:attribute name="customValue">TIMESTAMP</xsl:attribute>
							...
							<xsl:when test="@customValue ='DIM_LINK'">
								<xsl:attribute name="columnName"><xsl:value-of
									select="name()" /></xsl:attribute>
								<xsl:attribute name="withoutQuotes">true</xsl:attribute>
								<xsl:value-of select="." />
